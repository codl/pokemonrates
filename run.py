...
class Bot(mastodon.StreamListener):
    ...
    def on_mention(self, status):
        ...
        if pokémon_set:
            ...
            reply = self.tracery.flatten("#[pokemon:{}]origin#".format(pokémon))
            ...
            if not any(
                map(
                    lambda status: status["account"]["id"] == self.me["id"],
                    ctx["ancestors"],
                )
            ):
    ...
    def on_notification(self, notification):
        if notification["type"] == "mention":
            ...
            if reply:
                ...
                self.mastodon.status_post(
                    mentions + reply, in_reply_to_id=status["id"], visibility=visibility
                )
    ...
    def run(self):
        ...
        while True:
            ...
            if time.time() > self.config["last_timer"] + 60 * 61:
                is_public = (
                    time.time() > self.config["last_public_timer"] + 60 * 60 * 24
                )
